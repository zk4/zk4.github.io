...
# 1 基本命令
因为要涉及到类的调试，我们选用c++，首先准备好我们的调试文件test.cpp  如下
...
这3种point 在GNU中统称breakpoint--断点 。比较囧 ，当然。重点说1.breakpoint 的应用。

**举例用法**

1. break 3       //在当前调试的文件中的第3行停止程序 
2. break main   //在当前调试的文件中的symbol table中找main这样的名字。如果找到。则停止程序。 
3. break test.cpp:3    // 在test.cpp文件中的第3行停止程序 
4. break test.cpp:main  //在test.cpp文件的symbol table中找main这样的名字。如果找到。则停止程序。
...
## 断点基本管理
说到管理。我想做过数据库的都会想到 增删改查，那我们从4方面入手。
...
```
(gdb) d 1     //删除Num为1的断点。哪来的Num?? 输入 i b 在gdb中的断点列表的第一列！
(gdb) d 1 2 5 //可以同时删除多个断点
(gdb) d       //如果没有参数 则删除所有断点

(gdb) clear        //消除gdb当前行的断点. 
(gdb) clear main   //注意没有delete main这样的命令 clear main的意思是，清除函数main内所有的断点。
...
​	用list命令,同时你也可以用list命令切换你当前的命令作用对象.
...
````
(gdb) enable 1 //这里1代表的是i b 列表里的Num号
(gdb) disable 1 5
(gdb) enable once 1 5 // 只启用断点一起.中断后将置为disable状态
````
...
### 改
 ...
 ```
(gdb) cond 3 i >3 //将Num为3的断点加上i>3的条件,满足此条件时.停在此断点
 ```
